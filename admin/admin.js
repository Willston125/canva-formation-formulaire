...
(function () {
  ...
  /**
   * Dimensions à fournir pour cet emplacement, en pixels.
   *
   ...
   * l'essentiel du sujet se retrouvait hors cadre.
   *
   * Les deux nombres sont CALCULÉS : la largeur est celle à laquelle l'envoi
   * borne le fichier, la hauteur en découle par le rapport mesuré sur le site.
   * Sans rapport déclaré, on ne donne que la largeur plutôt qu'une hauteur
   * inventée.
   */
  function tailleConseillee(format, rapport) {
    var f = FORMATS_IMAGE[format];
    if (!f || !f.largeur) return '';
    var r = lireRapport(rapport);
    if (r === null) return f.largeur + ' px de large';
    return f.largeur + ' × ' + Math.round(f.largeur / r) + ' px';
  }
        ...
        return '<div class="bloc"><h2>' + echapper(g.nom) + '</h2>'
          + g.items.map(function (v) {
            ...
            return '<div class="visuel">'
              ...
              /* La taille attendue est donnée en PIXELS, pas sous un nom de
                 format : « paysage » ne dit pas quoi préparer, et on sortait
                 une photo du téléphone en espérant qu'elle tombe juste. */
              + champImage('visuel-' + v.cle, '', valeur || v.origine,
                (valeur ? '' : 'Visuel d’origine du site. ')
                + (tailleConseillee(v.format, v.rapport)
                  ? 'À fournir : ' + echapper(tailleConseillee(v.format, v.rapport)) + '. '
                  : '')
                + echapper(v.description))
